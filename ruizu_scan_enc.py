"""Native Ruizu MJPEG scan encoder (ctypes wrapper around libruizu_scan.so)."""
...
LIB = ROOT / "ruizu_scan" / "build" / "libruizu_scan.so"
QTABLES = ROOT / "ruizu_scan" / "ruizu.qtables"
...
def _ensure_qtables() -> Path:
    if QTABLES.is_file():
        return QTABLES
    from ruizu_jpeg import LUMA_Q, CHROMA_Q
...
def vendor_to_cjpeg_quality(vendor_quality: int) -> int:
    """IJG quality with Ruizu -qtables (scan matches official size)."""
...
def encode_cjpeg(
    ...
    """Encode via cjpeg + Ruizu quant tables + full AVI1 header rewrite (reference path)."""
...
class RuizuScanEnc:
    """Encode YUV420P frames to Ruizu-compatible MJPEG (623 B AVI1 header + scan)."""

    def __init__(self, width: int, height: int, quality: int = VENDOR_QUALITY_DEFAULT):
        self._lib = _load_lib()
        self._enc = self._lib.ruizu_scan_enc_open(width, height, quality)
        if not self._enc:
            raise RuntimeError("ruizu_scan_enc_open failed")
        self.width = width
        self.height = height
    ...
    def encode(self, yuv420: bytes) -> bytes:
        ...
        n = self._lib.ruizu_scan_enc_frame(
            self._enc,
        ...
        if n < 0:
            raise RuntimeError(f"ruizu_scan_enc_frame failed: {n}")
        return bytes(out[:n])

    def close(self) -> None:
        if self._enc:
            self._lib.ruizu_scan_enc_close(self._enc)
            self._enc = None

    def __enter__(self) -> RuizuScanEnc:
        return self
...
def vendor_to_ijg_quality(vendor_quality: int) -> int:
    lib = _load_lib()
    return int(lib.ruizu_scan_vendor_to_ijg_quality(vendor_quality))
...
def encode_yuv_file(
    ...
    with RuizuScanEnc(width, height, quality) as enc:
        jpeg = enc.encode(yuv)
    if out_path:
...
def _load_lib() -> ctypes.CDLL:
    ...
    if not LIB.is_file():
        raise FileNotFoundError(
            f"{LIB} not found — run: make -C {ROOT / 'ruizu_scan'}"
        )
    _lib = ctypes.CDLL(str(LIB))
    _lib.ruizu_scan_enc_open.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_int]
    _lib.ruizu_scan_enc_open.restype = ctypes.c_void_p
    _lib.ruizu_scan_enc_close.argtypes = [ctypes.c_void_p]
    _lib.ruizu_scan_enc_close.restype = None
    _lib.ruizu_scan_enc_frame.argtypes = [
        ctypes.c_void_p,
        ctypes.c_char_p,
        ctypes.POINTER(ctypes.c_uint8),
        ctypes.c_size_t,
    ]
    _lib.ruizu_scan_enc_frame.restype = ctypes.c_int
    _lib.ruizu_scan_vendor_to_ijg_quality.argtypes = [ctypes.c_int]
    _lib.ruizu_scan_vendor_to_ijg_quality.restype = ctypes.c_int
    return _lib
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="Ruizu native MJPEG scan encoder test")
    ...
    p.add_argument("-o", "--out", type=Path, default=Path("/tmp/ruizu_scan_out.jpg"))
